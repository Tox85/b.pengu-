...
console.log('🐧 Test Orca PENGU/WSOL simple...');

dotenv.config();

async function testOrcaPenguSimple() {
  try {
    // 1. Configuration
    console.log('\n1️⃣ Configuration...');
    ...
    if (penguWsolPoolId) {
      ...
      try {
        ...
        const pool = await client.getPool(new PublicKey(penguWsolPoolId));
        ...
        if (error.message.includes('Account does not exist')) {
          ...
          console.log('   2. Mettre à jour ORCA_PENGU_WSOL_POOL dans .env');
        }
      }
      
    } else {
      console.log('   ❌ Aucun pool PENGU/WSOL configuré');
      console.log('   Ajoutez ORCA_PENGU_WSOL_POOL=<pool_id> dans .env');
    }
    ...
    if (penguWsolPoolId) {
      ...
      if (hasEnoughPengu && hasEnoughWsol) {
        ...
        console.log('   Prochaine étape: Tester le LP avec orca-lp-pengu-wsol.js');
      } else {
        console.log('   ⚠️  Fonds insuffisants pour le LP');
        console.log('   Actions: Acheter plus de PENGU/WSOL via Jupiter');
      }
    } else {
